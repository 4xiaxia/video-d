...
- C 贴纸分叉对齐禁区①：`BoardHandwritingStickerContent`(手写体=简单式) vs `BoardMathStickerContent`(KaTeX=复杂式)，眼睛路两条腿。手写转换表已含 17 条映射，`\angle→∠`/`\pm→±`/`\perp→⊥` 等。
...
flowchart TD
    SEC[section] --> Z{落哪个区}
    Z -->|开场读题| ZO[题目区标签对应的分区<br/>opening boardSlice必须留空]
    Z -->|分析题目| ZA[分析区]
