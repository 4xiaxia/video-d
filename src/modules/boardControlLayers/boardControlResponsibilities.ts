...
export const boardControlResponsibilities: BoardControlResponsibility[] = [
  ...
  },
  {
    ...
    component: 'B 显示窗口',
    purpose: 'B 控制 C 何时上台；解锁后提供 C 的下台截止时间。',
    owns: 'startMs/endMs 的显示窗口，以及解锁后的 hideAtMs。',
    notOwns: '不代表 C 书写开始/结束；不改 A source；不管字体。',
    effect: 'B 决定 C 何时上台；默认不写 hideAtMs，C 自然播放完成后继续留场。解锁后才通过 hideAtMs 控制退场。A source 只决定动态书写落在哪段时间里。drawSpeed 只改书写快慢，不由 B 寿命隐式改写。',
    fields: ['startMs', 'endMs', 'hideAtMs'],
    frontend: 'VoiceTrack / 时间轴 B 寿命控件',
    uniqueness: '可拖动 B clip 或在时间轴主控精调；规则模块唯一：normalizeBoardDisplayWindow() + normalizeBoardHideAtMs()。',
  },
  {
    ...
    effect: 'revealStartMs/revealEndMs 是 A source 与 B 显示窗口归一化后的动态书写窗口缓存；它不决定 C 是否留场。C 默认写完继续留场；书写快慢仍由 drawSpeed 控制。',
