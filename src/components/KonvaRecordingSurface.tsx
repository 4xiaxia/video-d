...
const ZONE_LABELS = [
  { key: 'problem' as const, text: '题目' },
  { key: 'analysis' as const, text: '分析' },
  { key: 'solution' as const, text: '解答' },
  { key: 'summary' as const, text: '总结' },
] as const;

export function KonvaRecordingSurface({
  canvas,
  onCanvasReady,
  problemSummary,
}: {
  ...
  const problemMaxWidth = Math.min(
    canvas.width * COURSEWARE_PROBLEM_MAX_WIDTH_RATIO,
    520 * (canvas.width / 1120),
  );
  ...
  useEffect(() => {
    ...
    onCanvasReady?.(canvasEl as HTMLCanvasElement | null);
  }, [onCanvasReady]);
  ...
  return (
    <div
      ...
      aria-hidden="true"
      data-canvas-recording-surface="konva-foundation"
    >
      <Stage height={canvas.height} width={canvas.width}>
        <Layer listening={false}>
          <Rect fill={canvas.background || '#ffffff'} height={canvas.height} width={canvas.width} x={0} y={0} />

          <Rect
            height={canvas.height - frameStrokeWidth}
            stroke={FRAME_STROKE}
            strokeWidth={frameStrokeWidth}
            width={canvas.width - frameStrokeWidth}
            x={frameStrokeWidth / 2}
            y={frameStrokeWidth / 2}
          />

          {ZONE_LABELS.map(({ key, text }) => (
            <React.Fragment key={key}>
              <Rect
                ...
                height={labelHeight}
                width={labelWidth}
                x={canvas.width * COURSEWARE_LABEL_LEFT_RATIOS[key]}
                y={canvas.height * COURSEWARE_LABEL_TOP_RATIOS[key]}
              />
              <Text
                align="center"
                fill={LABEL_TEXT}
