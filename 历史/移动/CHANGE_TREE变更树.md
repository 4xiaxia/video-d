...
- 目标：
  - 把 C 板书降配的施工顺序写死，防止后续一急就跳步。
  - 当前只写“实施步骤/护栏”，不宣称代码已完成。
- 背景：
  - 甲方已确认当前主线降配为：`boardSlice` 普通多行文本 -> 保留用户换行 `\n` -> 手写字体 -> 逐字 reveal。
  - 旧 SVG/path/逐笔轨迹路线冻结备用，不删除，不作为当前施工依据。
- 安全顺序（必须按序）：
  1. **只读定位换行丢失点**：先查页面/舞台渲染层哪里把 `boardSlice` 的 `\n` 压没；不先大改文档，不先写新 layout。
  2. **抽纯文本 layout**：输入只允许来自 `boardSlice/fontSize/lineHeight/widthPercent` 等既有 C 参数；输出行数组、bbox、reveal 字符索引；加最小测试。
  3. **页面与录制共用 layout**：DOM 预览、Canvas 录制、未来 Konva 都消费同一 layout 结果；禁止各层各写一套排版。
  4. **文档同步已验证事实**：先登记本 `CHANGE_TREE`，再同步 `真相路标` / `认知图` / `ABC字段函数前端映射表`；文档只记录已验证链路，不提前固化猜测。
  5. **标签拆两刀**：第一刀只做 bbox + 3px 自适应；第二刀才做拖动 manual override。
- 标签边界：
  - 标签自适应不得改 `boardSlice`。
  - 标签拖动不得写回内容真相。
  - 若需要持久化，必须走独立字段，例如 `labelPlacement: { source: "auto" | "manual", ... }`，先设计再下笔。
- 最大风险：
  - 文档先大改会固化未验证假设。
  - DOM / Canvas / Konva 分头排版会制造第二真相。
  - 标签拖动污染 `boardSlice` 会破坏 C 内容真相。
- 当前状态：
  - 纯文档锚点已写入；代码未动。
- 下一枝：
  - 只读定位 `boardSlice` 多行文本在页面/舞台渲染层被压扁的位置。

## 2026-06-05_施工纪律更新：最低风险小步 + 每变更写 tree

- 目标：
  - 按夏夏要求，把后续施工纪律压实为“最低风险优先，宁可小心，不抢跑”。
  - 每次代码/文档变更后，都必须同步登记本 `CHANGE_TREE变更树.md`。
- 决策：
  - 先文档锚点，再小步代码；每次只改一个可验证切片。
  - 每刀改动前说明目标、边界、不可触碰项。
  - 每刀改动后记录：目标、文件变动、校验结果、下一枝入口。
  - C 板书降配主线当前不先引入 Agent，只预留未来排版矫正锚点。
- 已做：
  - 更新项目长期记忆与用户长期记忆中的安全施工偏好。
  - 本条作为后续 change tree 写入纪律的起点。
- 校验：
  - 纯文档/记忆纪律更新，无代码运行。
- 下一枝：
  - 先纠偏根目录 4 个真相文档，再查页面多行文本与标签 bbox 链路。
...
- 目标：
  - 全仓 X-ray 核心字段(boardSlice/voiceText/chainKey/xPercent/drawSpeed/reveal/goldenFinger)、验证文档认知与代码真相咬合度。
  - 数学符号「耳朵≠眼睛」两条路由实证确认，手写字体边界修正确认。
  - 死代码隔离：`TldrawStagePreview.tsx`(384行)、`TldrawProofPage.tsx`(243行)挪 `_deprecated/`。
  - 确保每一步都有 `npm run typecheck` 通过 + `npm run dev` 启动 + HTTP 200 验证。
- 方法：
  - 不靠记忆/猜测，全部 `grep` + `import` 链实证；字段逐一查"谁写/谁读/谁越权"。
- 实证结论（全量通过）：
  - **主舞台链**：`App → StagePreview(re-export) → LegacyStagePreview → DrawboardStage` ✅ 完全咬合，四区标签(题目/分析/解答/总结)硬编码在 DrawboardStage:104-107。
  - **tldraw 退场**：`TldrawStagePreview.tsx` 零外部引用 ✅；`TldrawProofPage.tsx` 仅 main.tsx standalone 路由引用 ✅；`BoardPreviewCard.tsx` 仍有一条活 tldraw 引用（import tldraw + resolveTldrawStageSize，去 tldraw 化未闭环）。
  - **ABC 三轨**：`globalRules.ts` 是单一真相源，被 abcChainKey/defaultConfig/ScriptAgentTableEditor 消费，无第二套定义。
  - **字段纪律**：boardSlice/voiceText 只在 ScriptAgentTableEditor 写，chainKey 唯一由 createRowChainKey 程序生成（Agent 不手写）✅；无组件层越权写入。
  - **数学耳眼分路**：`speechText/aliyunMathSpeechText.ts`(耳朵) ↔ `boardSticker/boardTextDisplayRoute.ts`(眼睛)，两个模块独立，共用一个 normalizeBoardMathText 但路由分叉 ✅。
  - **手写符号转换表**：`mathBoardText.ts:71-91` 含 17 条 LaTeX→Unicode 映射（\times→× / \div→÷ / \angle→∠ / \pm→± / \perp→⊥ 等），补全于 2026-06-02 症状B修复，已实证全部走手写字体路。
  - **死代码隔离**：`TldrawStagePreview.tsx` → `_deprecated/`；`TldrawProofPage.tsx` → `_deprecated/`；`main.tsx` 摘除 tldraw-proof 路由。
  - **CSS 统计校正**：`styles.css` 实际 2479 行（非文档所述的 2917），~535 px 硬编码（原称 604），91 行含 CSS 变量（原称 13 个变量名，口径不同）。
  - **金手指 overlay**：`GoldenFingerCanvasLayer` 用内部 state，不写 store 的 `goldenFingerOverlays` 字段；录屏靠 DOM querySelector 拿 canvas；边界正确（不写 ABC base），但类型定义 `goldenFingerOverlays` 是空壳。
  - **change_tree 位置**：文件在根目录 `CHANGE_TREE变更树.md`，不在 `历史/`（真相路标旧路径已过期）。
- 校验：
  - `npm run typecheck` 三次全部零错误通过（每步改动后即时验证）。
  - `npm run dev` 启动成功，HTTP 200 全页面（`/` / `?standalone=konva-proof` / `?standalone=c-sticker` / `?standalone=drawboard-core`）返回正常。
  - lint 零报错。
- 边界（未动）：
  - 不碰 BoardPreviewCard（tldraw 活尾巴，去 tldraw 化属后续技术债）。
  - 不删 `abcToTldrawShapes.ts`（BoardPreviewCard 仍消费 `resolveTldrawStageSize`）。
  - 不碰业务逻辑、不碰前三步主链。
- 下一枝：
  - BoardPreviewCard 去 tldraw 化（拔 `import tldraw` + `resolveTldrawStageSize`，换 Konva 承载预览）。
  - 或继续第3步 `voiceText→TTS→A轨真实时长` 走读。

## 2026-06-02_认知图新增 components 组件地图（无损压缩·mermaid）

- 目标：
  - 夏夏要求把 `src/components` 的认知沉进认知图，用 mermaid 做「上下文压缩但不失真」，动态维护。
- 方法（省 token，非读全文）：
  - 不读 49 个组件全文（烧 token）；只 grep 每个组件顶部 `@domain/@slot` 标签头（COMPONENT_TAGS.md 强制规范）。
  - 标签头=代码实证，非臆测。45 个 .tsx 组件全部有标签，数据干净。
- 已做（单文件单刀）：
  - `认知图-核心逻辑动态图.md` 在「一、全链总览」后插入新章节「一之B、components 组件地图」。
  - mermaid flowchart 按 @domain 分 10 个功能域子图（teaching-assets / script-agent-interface / voice-tts / teaching-timeline / stage-preview / drawboard-stage / board-sticker-rendering / math-content-rendering / inspector / settings-录制-归档）。
  - 沿用认知图既有 classDef done/warn 配色；stage-preview 标 warn（框架尾巴禁区⑤）。
  - 补 5 条关键认知，全部对齐 5 大禁区（StagePreview 入口 vs Tldraw 尾巴、画板三层不侵入、C贴纸手写vsKaTeX两条腿、只读控制索引、字体令牌跨域共用）。
- 校验：
  - 纯文档新增，无代码改动，无需 typecheck。
  - mermaid 语法沿用文件内既有 flowchart 风格（同 classDef）。
- 边界（未动）：
  - 不改组件代码，不改标签；纯认知沉淀。
  - 工具选型：graphify 太重（跑 pipeline 生成 HTML/JSON）放弃，只借「边标实证/推断」思路；mermaid 手画轻量动态长。
- 下一枝：
  - 新增/删组件时同步本图（看一点补一点）。

## 2026-06-02_眼睛路症状B修复：板书LaTeX命令残留

- 目标：
  - 夏夏确认症状为「B类=LaTeX 残留」：`\pm \angle \perp` 等命令原样显示在板书上（眼睛路/前端显示层问题，非语音/TTS 转义层）。
  - 只修前端显示转换表，不动判决逻辑、不碰语音路。
- 根因（代码实证）：
  - `src/modules/boardSticker/mathBoardText.ts:71-75` `normalizeElementaryBoardHandwritingText` 转换表原只有 4 条（`\left/\right`删、`\times→×`、`\div→÷`、`\cdot→·`）。
  - 漏网命令既不在转换表、又不在 `LATEX_HINT_PATTERN`(19-20行)公式判定表 → 既不转符号也不判公式 → 原样吐出残留。
  - 反差：`HANDWRITING_EXTRA_SYMBOLS`(29行)白名单早已含 ∠ ± ∓ ° ⊥ ∥ △ ⊙ ∵ ∴ ∝，手写字体能写，只是没人把对应 LaTeX 命令转过去。
- 已做（单文件单刀，纯增量映射）：
  - `mathBoardText.ts:76-91` 在原 4 条 replace 后补 13 条映射：`^\circ`(上标，先于裸`\circ`处理)、`\circ`、`\degree` → `°`；`\angle→∠`、`\pm→±`、`\mp→∓`、`\perp→⊥`、`\parallel→∥`、`\triangle→△`、`\odot→⊙`、`\therefore→∴`、`\because→∵`、`\propto→∝`。带 `@xiaxia-2026-06-02` 锚点注释。
- 校验：
  - `npm run typecheck`(tsc --noEmit) 通过，无回归。
  - 专项验证(临时脚本，已删)：11 个症状B样例（`\angle ABC=90^\circ`、`x=5\pm 2`、`AB\perp CD` 等）全部从 LaTeX 残留转为 Unicode 符号并走 handwriting 路，无 `\xxx` 残留。
  - 注：`npm run check:board-handwriting-support` 因脚本 hardcode 了缺失的 `runtime/node/node.exe`(USB便携环境遗留)无法直接跑，与本改动无关；已用当前 Node 复刻其编译+断言流程通过。
- 边界（未动）：
  - B类符号(`\le \ge \neq \approx \infty \pi`)在公式表里所以不残留，但用印刷体渲染了本可手写的符号（属症状C，本次不动）。
- 下一枝：
  - 可选：症状C（`\le`等本可手写却走印刷体）是否处理；或继续第3步 `voiceText→TTS→A轨真实时长` 走读。

## 2026-06-02_文档收敛五份制：归档冗余+统一引用

- 目标：
  - 夏夏要求说明文档不要散落多份，按 5 份核心模型收敛：1认知(真相路标) 2图谱(认知图) 3关系详解(ABC映射表) 4说明文档(代码直敲版) 5tree(变更树)。
- 已做：
  - `落地项目开发说明明细文档.md` → 移至 `历史/`（内容与代码直敲版重复且未止血）。
  - `当前项目全局认知图与批注说明.md` → 移至 `历史/`（功能与认知图重叠）。
  - `代码直敲版.md` 第9行「当前真相文件」引用更新为认知图。
  - `ABC映射表.md` 第9行「当前真相文件」引用同步更新。
  - `CHANGE_TREE变更树.md` 第二刀下一枝标注「已归档无需再处理」。
- 根目录核心文档（5+4）：
  - 5份动态维护：真相路标 / 认知图 / ABC映射表 / 代码直敲版 / tree
  - 4份项目辅助：CLAUDE.md / ONBOARDING.md / PRECHECK.md / README.md
- 下一枝：
  - 继续第3步 `voiceText→TTS→A轨真实时长` 走读；或处理 BoardPreviewCard 去 tldraw 化尾巴。

## 2026-06-02_tldraw文档止血第一刀：主舞台路径纠偏

- 目标：
  - 夏夏指出文档里仍有旧 tldraw 口径，容易误导为当前项目继续以 tldraw 为主。
  - 只做文档止血，不碰业务代码。
- 已做（纯文档，未碰业务代码）：
  - `当前项目全局认知图与批注说明.md`：纠正主舞台公共入口，当前路径改为 `StagePreview.tsx -> LegacyStagePreview.tsx -> DrawboardStage.tsx / AutoHandwritingLayer.tsx`。
  - `当前项目全局认知图与批注说明.md`：更新 Mermaid 链路图，移除当前主路中的 `TldrawStagePreview.tsx / abcToTldrawShapes.ts`，把它们降为遗留退场线。
  - `当前项目全局认知图与批注说明.md`：更新“需要替换的”段落，避免把 `TldrawStagePreview` 写成当前舞台适配器。
  - `ABC字段函数前端映射表.md`：把 C 演员文本与站位消费端从 `TldrawStagePreview` 改回当前实际消费层 `AutoHandwritingLayer`，把 `tldraw shape patch` 改为 `C视觉 patch`。
  - `ABC字段函数前端映射表.md`：待补齐项从“tldraw 主舞台金手指隔离”改成“overlay 金手指隔离”。
- 实证依据：
  - `src/components/StagePreview.tsx:4`：当前导出 `StagePreview` from `LegacyStagePreview`。
  - `src/components/LegacyStagePreview.tsx:17-18`：当前主舞台调用 `AutoHandwritingLayer` 与 `DrawboardStage`。
  - `src/components/DrawboardStage.tsx:13,94-103,116-123`：当前 stage/chrome/base/overlay 层路径。
  - `src/components/AutoHandwritingLayer.tsx:60-64,101-109`：当前 C 字段消费层。
- 当前边界：
  - `真相路标` 和 `认知图` 中写“tldraw 退场 / tldraw 尾巴 / tldraw→Konva 决策线”的内容不是旧错，是当前真相，保留。
  - `落地项目开发说明明细文档*.md` 中仍有大量 tldraw 旧段落，但文件头已标“已被覆盖”；后续可按横表批注规则逐步止血，不一次性重写。
- 下一枝：
  - 继续第3步 `voiceText→TTS→A轨真实时长` 与 `B寿命窗口生成` 走读；或继续做旧文档 tldraw 批注止血第二刀。

## 2026-06-02_tldraw文档止血第二刀：代码直敲版全量批注

- 目标：
  - 夏夏指出 `落地项目开发说明明细文档-代码直敲版.md` 没有及时更新，仍有大量 tldraw 旧表述当成当前施工方针。
  - 逐段批注止血，不重写正文，保留旧施工图历史。
- 已做（纯文档批注，未碰业务代码）：
  - 文件头 3-13 行批注框架：已标注「已被覆盖 / 为什么覆盖 / 下一步」。
  - §0.3 步实现清单（36 行）：`tldraw 承载` 加批注「去 tldraw 化中」。
  - §2 技术栈（68 行）：`tldraw 5` 加纠偏「已退场」。
  - §2 原则区（72/74/76 行）：`tldraw-first / 预览统一靠 tldraw` 整体作废加引路链接。
  - §2.1.1 组件树（97 行）：`TldrawStagePreview` 纠正为 `LegacyStagePreview`。
  - §2.1.1 预览样式说明（139 行）：BoardPreviewCard 标注「仍有 tldraw size 尾巴」。
  - §2.1.1 坐系前缀（160 行）：作废「预览和主舞台都使用 tldraw 承载」。
  - §3.2 标题（237 行）：去 `tldraw 承载边界` +标注以 drawboard/Konva 为准。
  - §3.2.3 整节（261-268 行）：`tldraw 承载策略（框架优先）` 整节作废批注。
  - §5.1 VoiceWorkspace 约束（402 行）：`tldraw shape 转换` → `shape 转换` +去 tldraw 化标注。
  - §5.5 BoardPreviewCard 改造（451 行）：`只读 tldraw 画布` → `只读画布渲染` +纠偏。
  - §7.2 建议新增文件（526 行）：`layoutPreviewToTldrawShapes.ts` 作废，标注不再新建 tldraw 转换模块。
  - §8 注意事项（542 行）：`预览也走 tldraw 承载` 改为「统一承载」+纠偏。
  - §11 Mermaid 时序图（685 行）：BoardPreviewCard(tldraw) 去括号 +标注去 tldraw 化。
  - §15 整节（734-753 行）：`与 tldraw 能力的结合点（优先复用）` 整节作废，§15.1 四条 tldraw API 全部作废并替换为 Konva 对应，§15.2/15.3 意图保留但去 tldraw 表述。
  - §17 控件表 VoiceWorkspace 行（790 行）：`tldraw shape 构建` → `画布节点构建`。
  - §17 控件表 BoardPreviewCard 行（791 行）：`tldraw shape` → `画布节点` +纠偏。
  - §17 控件表 TldrawStagePreview 行（792 行）：整行标注为死代码，补现役 StagePreview→LegacyStagePreview 行。
  - §19.3 金手指隔离（815 行）：`当前主舞台策略=保持 tldraw 画笔` 作废，补 GoldenFingerCanvasLayer 承担说明。
  - §21 节点复盘标题（843 行）：加过期标注，说明当时基于 tldraw。
  - §21.2 复盘表 TldrawStagePreview 行（855 行）：补注「此补丁随 tldraw 退场失效」。
  - §22.2 节点B 标题（877 行）：打作废线 +过期标注。
  - §22.2 节点B 第4条（885 行）：`画笔向 tldraw 靠拢` 作废标注。
- 实证依据：
  - grep 实证：`TldrawStagePreview.tsx` 仅被自身引用、无人 import（死代码）。
  - `StagePreview.tsx` re-export 到 `LegacyStagePreview`。
  - `DrawboardStage.tsx` + `AutoHandwritingLayer.tsx` 为当前主舞台渲染链。
  - `GoldenFingerCanvasLayer.tsx` 自有 canvas 承担画笔/橡皮 overlay。
- 当前边界：
  - 文件头「已被覆盖」批注与各段纠偏共同成立，施工从此以后不按 tldraw 底座执行。
  - BoardPreviewCard 去 tldraw 化未闭环（剩 tldraw size 尾巴），属于技术债，已在批注中标明。
  - §21/§22 节点复盘内容属历史记录，保留原文但不作当前施工依据。
- 下一枝：
  - `落地项目开发说明明细文档.md` 同类 tldraw 止血（注：已归档至 `历史/`，无需再处理）；继续第3步 `voiceText→TTS→A轨` 走读。

- 目标：
  - 夏夏决定先不纠结 WorkBuddy 侧 graphify/imptokens 安装，回到历史与可维护文档。
  - 继续维护三份动态文档：`真相路标-当前唯一入口.md` / `认知图-核心逻辑动态图.md` / `CHANGE_TREE变更树.md`。
  - 保持原则：只补已实证内容，不碰业务代码。
- 已做（纯文档，未碰业务代码）：
  - 重新确认根目录主 tree 真身是 `CHANGE_TREE变更树.md`，不是 `历史/CHANGE_TREE变更树.md`。
  - 读取 `真相路标-当前唯一入口.md`、`认知图-核心逻辑动态图.md`、主 tree，确认当前断点。
  - `认知图-核心逻辑动态图.md` 雷区①补入耳朵路踩坑防线：`repairCommonLatexEscapeDamage()` 会修复被转义损坏的 `\frac`（如 `\u000c rac` / `rac{}` / `frac{}`），再进入阿里云数学语音链。
- 实证依据：
  - `src/modules/speechText/aliyunMathSpeechText.ts:13-14`：`prepareAliyunMathSpeechText()` 入口先调用 `repairCommonLatexEscapeDamage(text)`。
  - `src/modules/speechText/aliyunMathSpeechText.ts:51-55`：函数修复 `\u000c rac`、`rac{}`、`frac{}` 为 `\frac`。
- 当前边界：
  - 不改业务代码。
  - 不继续纠结技能迁移；graphify 已复制到 WorkBuddy skill 目录但可运行性未闭环，后续若要用再单独处理。
  - 不把数学双路重复写成第二真相，只给认知图雷区①补真实缺口。
- 下一枝：
  - 继续按认知图 `五、待走读补图区` 走：优先第3步 `voiceText→TTS→A轨真实时长` 与 `B寿命窗口生成`，再走第4步舞台链。

## 2026-05-31_勘探与止血：路标重建+全局规矩收口+第一张骨牌走读

> 说明：变更树自 2026-05-06 断了 25 天（中间改动没人登记，是文档/认知被搞乱的根因）。今天把树接上。本刀以勘探+止血为主，不改业务代码（typecheck 全程通过）。

- 目标：
  - 程序交付日，先确认可交付，再把被熊孩子搞乱的"指南针"校准。
  - 用代码 grep 实证真相，杜绝想当然猜测。
  - 把藏在里面的核心逻辑（字段贯穿/画布操纵杆/ABC链）显式落笔。
- 已做（全部可逆/纯文档，未碰业务代码）：
  - 新增根目录唯一路标 `真相路标-当前唯一入口.md`（从 历史/ 提到根目录并重命名）。内含：病灶诊断书 + 埋线地图 + 增量阅读入口。
  - 全局 `C:/Users/admin/.claude/CLAUDE.md` 顶部新增"出发主轴·施工图纸"（总纲+七铁律+四件套），并把 13 份散落规则 md 归档到 `backups/散文档归档-2026-05-31/`（可逆）。
  - 清理对话残渣：`对话 能找到历史记录吗.md` 移入 历史/ 杂物间。
  - 起认知图 `认知图-核心逻辑动态图.md`（结构脑图，动态维护，看一点补一点）。
- 实证结论（grep + import 链 + Playwright 走读）：
  - 程序可交付：typecheck 通过，主链 App 不碰 tldraw。
  - 第一张骨牌（rows 表格）字段真相：section/stepLabel/boardSlice/voiceText/chainKey，与 `ABC字段函数前端映射表.md` 咬合。
  - C 分片→画布区域：section 定四区，xPercent/yPercent 定区内坐标，chrome 只画底板不碰 ABC。
  - 「讲解生成」链路通：输入题文→拉起对话面板→agent 生成 5 行切片（候选态有缓存，关闭不丢，确认才入正式）。
- 当前边界（今天不动，列为交付后专项）：
  - tldraw 尾巴（BoardPreviewCard）、CSS 令牌化、金手指/录屏解耦、参数覆盖链补中层——详见路标病灶诊断书。
- 已验证：
  - `npm run typecheck` 通过（走读前后各一次，未碰坏代码）。
  - Playwright 走读：页面 5196 可起、四步结构正确、生成链路通。
  - 临时探针脚本已清理，无残留。
- 下一枝：
  - 继续走第四步舞台命脉（LegacyStagePreview→DrawboardStage→AutoHandwritingLayer），边走边补认知图。
...
## 2026-06-06_舞台内样式分家到 stage.css

- 目标：
  - 按用户边界把画布内元素样式单独成册，先实现“画布是画布，其他是其他”。
- 已做：
  - 新增 `src/stage.css`，承接舞台内样式：
    - `stage-canvas`
    - `courseware-label`
    ...
    - `board-text-sticker`
    - `c-standalone-canvas`
    - hybrid prototype 的舞台壳子规则
  - `src/main.tsx` 接入 `stage.css`。
  - `src/styles.css` 删除舞台内样式，退回页面壳子与非舞台区域。
  - `scripts/check-board-boundaries.mjs` 改为同时读取 `styles.css` 与 `stage.css`，新增“舞台样式不得回流到全局样式表”的守门。
- 边界：
  - 本刀只做样式边界分家，不宣称标签 / 题目 / 板书的尺寸真相已经统一。
  - 不改第二步 rows 合同，不改 C 文本换行逻辑，不改业务时间线。
- 校验：
  - `npm run check:board-boundaries` -> 通过。
  - `npm run typecheck` -> 通过。
- 下一枝：
  - 继续收标签、题目、板书三条尺寸链，明确与画布的比例真相是否统一到同一来源。

## 2026-06-06_题图入口收成紧凑上传rail

- 目标：
  - 把第 1 步题图入口从大拖拽面板改成“左缩略 / 右按钮”的紧凑工具位。
- 已做：
  - `src/components/ProblemUploadPreview.tsx`
    - 从 `Upload.Dragger` 改为普通 `Upload`。
    - 空态与有图态统一成 `problem-upload-rail` 结构。
    - 保留原 `beforeUpload -> onImportProblemImage` 导入链路。
  - `src/styles.css`
    - 新增 `problem-upload-*` 紧凑 rail 样式。
    - 删除旧大拖拽框样式。
    - 题图缩略改成 `object-fit: contain`，避免卡片裁图。
  - `PROJECT_STATE.md`、`DECISIONS.md`、`KNOWN_ISSUES.md`、`ENGINEERING_LOG.md`
    - 记录本轮 UI 决策、风险和验证结果。
- 边界：
  - 不改识别逻辑，不改素材流，不改第二步 rows 合同，不改 C 渲染链路。
  - 不为样式新增多层无意义包裹；保持组件结构扁平。
- 校验：
  - `npm run typecheck` -> 通过。
  - `npm run check:board-boundaries` -> 通过。
  - `npm run smoke:pending-new-problem` -> 通过。
  - Playwright 空态 / 有图态实测 rail 高度均为 `104px`。
- 下一枝：
  - 继续回到标签 / 题目 / C 板书与画布比例同源的尺寸来源收口。

## 2026-06-06_标签分片容器按真实内容 bbox 自适应

- 目标：
  - 把四区标签从静态 chrome 改成“标签句柄 + 内容自适应分片容器”，并守住“标签拖动不撞板书拖拽”的交互边界。
- 已做：
  - `src/modules/canvasStage/coursewareZoneLayout.ts`
    - 新增四区分片布局 helper。
    - 从运行态 DOM 的 `stage-problem-text` 与 `board-text-sticker--zone-*` bbox 生成分区框和标签位置。
    - 内容 bbox 默认外扩 `5px`。
  - `src/components/BoardTextSticker.tsx` / `src/components/CStickerFrame.tsx` / `src/components/AutoHandwritingLayer.tsx`
    - 给每个 C 板书挂 `zoneKey` 和 `board-text-sticker--zone-*` class。
  - `src/components/DrawboardStage.tsx`
    - 渲染动态分区框。
    - 标签 pill 可拖，本地预览 override 不写回业务真相。
    - 容器框只显示，不接管指针。
  - `src/components/CanvasRecordingSurface.tsx` / `src/modules/canvasStage/drawCoursewareStageFrame.ts`
    - 录制底图同步消费同一份分区框与标签位置。
  - `src/stage.css`
    - 新增 `courseware-zone-box`。
    - 标签提升到高于板书的层级，避免“拖标签命中板书”。
- 边界：
  - 不改第二步 rows 合同。
  - 不写 `boardSlice`。
  - 不把标签拖动持久化成 manual override 字段。
  - 不让整个容器可拖，只允许标签 pill 承接拖拽。
- 校验：
  - `npm run typecheck` -> 通过。
  - `npm run check:board-boundaries` -> 通过。
  - 本地 Playwright `standalone=drawboard-core` 交互回归 -> 通过。
    - 拖标签期间板书位移 `0,0`
    - 板书拖动位移 `+40,+13`
    - 截图：`.tmp-ui-smoke/zone-label-drag-2026-06-06T06-12-59-865Z.png`
- 下一枝：
  - 继续把题目区 bbox、录制底图、Konva proof 页面收成同一套尺寸口径。

## 2026-06-06_右栏职责表容器收回 inspector 边界

- 目标：
  - 修复右侧 `A/B/C 控制层职责表` 展开后横向撑爆、纵向无限拉长的问题。
- 已做：
  - `src/styles.css`
    - 为 `.inspector-stack` 增加单列宽度约束。
    - 为 `.board-control-responsibilities-collapse` 及其 Ant 展开层补齐 `min-width: 0`、`width: 100%`。
    - 把展开内容的滚动边界挂到真实的 `.ant-collapse-body`，限制为侧栏内滚。
    - 收紧职责卡片内边距、间距，并让标题、事实字段和 tag 支持断行。
- 边界：
  - 不改 `boardControlResponsibilities` 文案真相。
  - 不改任何舞台内元素。
  - 不新增组件套层；继续保持现有扁平结构。
- 校验：
  - `npm run typecheck` -> 通过。
  - `npm run check:board-boundaries` -> 通过。
  - 本地 Playwright 右栏容器回归 -> 通过。
    - `workspace-sider--inspector` 无横向溢出：`scrollWidth == clientWidth == 294`
    - 职责表 `.ant-collapse-body` 高度约 `600px`，内部滚动生效
...
## 2026-06-07_分片chrome扁平模块与C自由画布实时文本

- 目标：
  - 按用户纠正，把“标签 + 容器 + 分片内容”收成扁平可复用 chrome 模块。
  - 解除旧固定四区站位限制，让 C 的 `xPercent/yPercent` 回到整张画布自由百分比坐标。
  - 修正题目正文样式和普通 C 字体变异问题。
- 已做：
  - `src/components/CoursewareSegmentChrome.tsx`
    - 新增扁平分片 chrome 模块，只渲染标签 pill 与可选容器框。
    - 容器 `pointer-events: none`，题目区不显示蓝色虚线框。
    - 增加 `data-agent-anchor` / `data-agent-zone`。
  - `src/components/DrawboardStage.tsx`
    - 用 `CoursewareSegmentChrome` 统一渲染四个分片 chrome。
  - `src/components/AutoHandwritingLayer.tsx`
    - 移除 `COURSEWARE_ZONE_BOUNDS` / `constrainYPercentToZone`。
    - 普通 C 录制改为实时文字绘制；复杂公式仍走公式图片路线。
  - `src/components/BoardHandwritingStickerContent.tsx`
    - 普通 C 页面预览改成 DOM 实时文本，不再生成 PNG。
  - `src/modules/canvasStage/coursewareChrome.ts` / `drawCoursewareStageFrame.ts` / `src/stage.css`
    - label 字号同源；题目字号 = label * `1.5`；题目正文 `font-weight: 400`。
    - 录制底图不画题目区虚线框。
  - `scripts/check-board-boundaries.mjs`
    - 新增守门：普通 C 不走 PNG、C 不回固定四区 clamp、分片 chrome 保持扁平。
  - `scripts/check-board-handwriting-support.mjs`
    - 本地 Node runtime 不存在时回退 `process.execPath`。
- 边界：
  - 不改第二步 rows 合同。
  - 不把标签拖动持久化。
  - 不删除 `renderBoardTextStickerImage` 备用模块。
  - 不把 C 内容包进新容器，不新增 DOM 套娃。
  - 不改公式/结构数学路线。
- 校验：
  - `npm run typecheck` -> 通过。
  - `npm run check:board-boundaries` -> 通过。
...
- 下一枝：
  - 抽普通 C DOM/录制共享文本 layout helper，继续收“实时文本同源”。
  - 若继续做标签锚点持久化，先设计独立字段，禁止写回 `boardSlice` / clip label。

## 2026-06-07_Konva主舞台方向接手审计与gate定界

- 目标：
  - 接手用户对 Konva 主控方向的最新要求，先核当前项目真相，再决定是否主舞台一刀全切。
  - 把“Konva 是标准方向”和“当前 proof 不能冒充生产链”写入接力记录。
- 已做：
  - 复核 `package.json`：项目为 React 19 + Vite + TypeScript + antd + zustand + `konva` / `react-konva`。
  - 复核 `src/main.tsx`：存在 `standalone=konva-proof` 独立 proof 入口。
  - 复核 `src/components/StagePreview.tsx` / `LegacyStagePreview.tsx`：当前公共舞台入口仍是 `LegacyStagePreview`。
  - 复核 `src/components/DrawboardStage.tsx` / `AutoHandwritingLayer.tsx`：当前生产主链是 `LegacyStagePreview -> DrawboardStage -> AutoHandwritingLayer`。
  - 复核 `src/components/KonvaRecordingSurface.tsx` / `src/standalone/KonvaProofPage.tsx`：当前 Konva 文件为 proof / 迁移口径，不是生产主链。
  - 复核 `src/modules/stageRecorder/useCanvasRecorder.ts`：录制仍按 base/content/overlay 三 canvas 合成。
  - 拉只读 sub agent `Godel` 做事前审视，结论同本地审计：今天不应盲切主舞台，应先记录 gated migration。
  - 更新 `DECISIONS.md`、`PROJECT_STATE.md`、`KNOWN_ISSUES.md`、`CHANGE_TREE变更树.md`、`ENGINEERING_LOG.md`。
- 边界：
  - 本枝不改业务代码，不改第二步 rows 合同，不把普通 C 改回 PNG。
  - 不把 `KonvaProofPage` 的 sample 数据接入主工作台。
  - 不恢复固定四区 clamp。
  - 不动未跟踪的 `历史/skills 要用啊/graphify`。
- 校验：
  - `npm run typecheck` -> 通过。
  - `npm run check:board-boundaries` -> 通过。
  - `npm run check:board-handwriting-support` -> 通过。
  - `npm run check:continuity-docs` -> 通过。
  - `node scripts/audit-local-order.mjs` -> `NEEDS ORDERING`，缺 `graphify`、`codesight`、`ccwf`、`ccwf-mcp`。
- 下一枝：
