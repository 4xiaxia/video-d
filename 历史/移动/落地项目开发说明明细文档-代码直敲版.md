...
| 仍然有效 | 第三步临时预览态的身份、`rows -> TTS -> BoardEvent -> TimelineClip(kind='board') -> StagePlayback` 正式链路、四区标签必须存在、预览不写正式 timeline 真相、统一配置/边界/验收纪律，这些仍然有效。另：ABC 关系上，A 是主时钟，B 是寿命/站场控制轨，C 是板书演员；C 自然播放完后默认留场，这条产品语义仍然有效且必须守。 |
...
## 2026-06-04 批注（ABC 语义纠偏）

- 第四步问题2此前被误解成“B 结束后 C 额外静态留场 1 秒”。这是错误口径。
- 正确口径：本项目是直播仿板书在线解题，C 板书内容自然播放完成后默认留在画布上。
- B 的能力仍保留，但它是“解锁后控制 C 站场时长”的轨，不是默认让 C 自然下台的轨。
- 后续如果文中出现“C 到 endMs 自然消失”或“静态留场 1 秒即可”等表述，都应视为过期语义，以 `PROJECT_COGNITION.md / 真相路标 / 认知图 / ABC映射表` 新口径为准。
...
---
  ...
  3. 新增临时预览状态（store）
  4. 侧边预览卡展示整版可视化结果（tldraw 承载）〔2026-06-02 纠偏：承载体去 tldraw 化中，BoardPreviewCard 仍有 tldraw size 尾巴待拔〕
- 不在本轮：
...
- Zustand 5
- Ant Design 6
- tldraw 5 〔⚠️2026-06-02 纠偏：tldraw 已退场，不再是必需技术栈。当前主舞台=drawboard 路线(LegacyStagePreview→DrawboardStage/AutoHandwritingLayer)，目标承载=Konva。tldraw 仅剩 BoardPreviewCard 活尾巴待拔 + 死代码待挪。〕
- Vite dev middleware（本地网关）

原则：
> 【2026-06-02 已过期·纠偏】以下「tldraw-first / 预览统一靠 tldraw」是旧阶段口径，已被覆盖。当前真相：本项目是 agent workflow，tldraw 太重且 shape/store 模型挡着 agent 直接控制字段，已决策退场换 Konva。原则保留下面两条仍有效的（不另起渲染系统、禁手搓三无运行时），tldraw-first 那条作废。详见 `真相路标-当前唯一入口.md` §3、`认知图-核心逻辑动态图.md` 四之B。

- ~~预览渲染统一靠 tldraw~~ 〔作废，预览卡正逐步去 tldraw 化〕
- 不新建第二套 HTML 绝对定位渲染系统 〔仍有效〕
- ~~tldraw-first：以 tldraw 作为统一画布框架与标准转换底座，优先复用其 已决策退场换 Konva。原则保留下面两条仍有效的（不另起渲染系统、禁手搓三无运行时），tldraw-first 那条作废。详见 `真相路标-当前唯一入口.md` §3、`认知图-核心逻辑动态图.md` 四之B。shape/store/ui 能力做项目转换~~ 〔作废：不再以 tldraw 为底座，承载层换 Konva〕
- 禁止扩散手搓三无运行时（无稳定契约、无回归基线、无可维护边界）的临时实现 〔仍有效〕
...
│  ├─ main: StagePreview(→LegacyStagePreview) + TeachingTimeline  〔2026-06-02 纠偏：StagePreview 现 re-export 到 LegacyStagePreview，不再是 TldrawStagePreview〕
...
- 标题：`板书排版预览（临时）`
- 说明：`用于视觉评审，不写正式C真相`
- 主体：只读预览（BoardPreviewCard）〔2026-06-02 纠偏：仍有 tldraw size 尾巴待拔，目标去 tldraw 化〕
...
- 画布对象：`%`（x/y/width）
- 禁止同语义混用绝对与相对单位
- ~~预览和主舞台都使用 tldraw 承载，避免两套视觉系统~~ 〔2026-06-02 作废：主舞台已是 drawboard 路线、目标 Konva；预览卡去 tldraw 化中。本条「避免两套视觉系统」的意图保留，但承载体不再是 tldraw〕
...
## 3.2 本地保存、缓存策略与承载边界〔2026-06-02 纠偏：原标题「tldraw 承载边界」，tldraw 已退场，承载边界以 drawboard/Konva 路线为准〕
...
### 3.2.3 ~~tldraw 承载策略（框架优先）~~〔2026-06-02 整节作废·纠偏〕

> 【已过期】本节原把 tldraw 当统一画布框架底座。当前真相：tldraw 退场，主舞台=drawboard 路线(LegacyStagePreview→DrawboardStage/AutoHandwritingLayer)，目标承载=Konva。下面旧条文仅留作历史，不作施工依据。仍然有效的只有「业务真相由 TeachingProject+store 承载」「禁止再造第二套平行渲染系统」两条。

- ~~tldraw 是统一画布框架底座（stage/preview 承载）~~ 〔作废〕
- 业务真相仍由 `TeachingProject + store` 承载 〔仍有效〕
- ~~`persistenceKey` 是编辑器级记忆，不抢业务真相主导权~~ 〔作废：tldraw editor 概念，已不适用〕
- 禁止再造第二套平行渲染系统 〔仍有效〕
...
不做：
- 不在这里写布局算法
- 不在这里做 shape 转换 〔2026-06-02 纠偏：原文「tldraw shape 转换」，承载体去 tldraw 化中〕
...
- 内部只读画布渲染整版 preview 〔2026-06-02 纠偏：原文「只读 tldraw 画布」，承载体去 tldraw 化中，目标 Konva〕
- 若无数据显示 placeholder
...
3. ~~`src/modules/tldrawStage/layoutPreviewToTldrawShapes.ts`~~ 〔2026-06-02 作废：不再新建 tldraw shape 转换模块，预览渲染走 drawboard/Konva 路线〕
...
4. **不要新增第二套渲染真相**
   - 预览也走统一承载 〔2026-06-02 纠偏：原文「走 tldraw 承载」，统一承载体已切 drawboard/Konva；本条「不要两套渲染真相」意图保留〕
...
sequenceDiagram
    ...
    participant BP as BoardPreviewCard〔预览卡·去tldraw化中〕
...
## 15. ~~与 tldraw 能力的结合点（优先复用）~~ 〔2026-06-02 §15.1 整体作废·纠偏〕

> 【已过期】本节原指引"优先复用 tldraw editor API"。当前真相：tldraw 退场，承载换 drawboard/Konva。下列 tldraw API（createShapes/zoomToBounds/toRichText）不再作为复用目标，改由 Konva 对应能力承担。§15.2「不重复造轮子」意图保留，§15.3 字段建议与框架无关、仍可参考。

### 15.1 ~~必复用能力~~ 〔作废：以下均为 tldraw editor API，不再复用〕
- ~~`Tldraw` 只读画布承载预览~~ → 改 drawboard/Konva 画布承载
- ~~`editor.createShapes` 批量创建文本 shape~~ → 改 Konva 文本节点批量创建
- ~~`editor.zoomToBounds` 自动适配整版可见区域~~ → 改 Konva stage scale/适配
- ~~`toRichText` 文本入 shape 规范化~~ → 改 Konva 文本规范化

### 15.2 不要重复造轮子 〔仍有效，与框架无关〕
- 不再单独造 HTML 绝对定位排版系统
...
| VoiceWorkspace | 第三步动作中枢 | 生成排版预览、生成讲解音频 | 不做布局算法本体，不做画布节点构建〔2026-06-02 纠偏：原「tldraw shape 构建」，承载体去 tldraw 化中〕 | 点击 -> 请求 -> 状态更新 | 受 scriptAgentCandidateDraft、ttsConfig、stageCanvas 影响 | rows / stageCanvas / scriptAgentConfig / ttsConfig | `src/components/VoiceWorkspace.tsx` | 生成板书排版预览 / 生成讲解音频 | 是 |
| BoardPreviewCard | 侧边整版预览 | 只读显示临时排版预览 | 不写 timeline，不做业务决策 | layoutPreviewDraft -> 画布节点 -> 画面〔2026-06-02 纠偏：原「tldraw shape」，去 tldraw 化中〕 | 受 draft items 与 canvas 比例影响 | draft.version/items、stageCanvas | `src/components/BoardPreviewCard.tsx` | 板书排版预览（临时） | 是 |
| ~~TldrawStagePreview~~〔2026-06-02 死代码·已退役〕 | ~~主舞台承载~~ | — | — | — | — | — | ~~`src/components/TldrawStagePreview.tsx`(385行)~~ 已是死代码，主舞台真路径=`StagePreview→LegacyStagePreview→DrawboardStage/AutoHandwritingLayer` | 预览舞台 | 否 |
...
- ~~当前主舞台策略：保持 tldraw 原生画笔/橡皮能力，并通过受管形状缺失修复保护 ABC 基础元素~~ 〔2026-06-02 纠偏：tldraw 已退场，「tldraw 原生画笔/橡皮 + 受管形状修复」是旧 tldraw 主舞台口径，已作废。当前金手指 overlay 标注能力由 `GoldenFingerCanvasLayer` 自有 canvas 承担，与底层渲染框架(drawboard/Konva)解耦；"只在 overlay 层、不回写 base A/B/C"的隔离纪律仍有效〕
...
## 21. 节点复盘（2026-05-29）〔2026-06-02 注：本节为历史复盘记录，当时主舞台仍基于 tldraw，下文「TldrawStagePreview 工具保护补丁」等叙述是当时事实，现已过期——TldrawStagePreview 已成死代码，主舞台切至 drawboard/Konva 路线〕
...
### 22.2 ~~节点B：恢复 tldraw 原生能力并保留保护边界~~ 〔2026-06-02 注：以下为历史复盘记录。tldraw 已退场，"恢复 tldraw 原生能力/画笔靠拢"等目标不再适用；"保护底层真相不受误删"的纪律仍在 GoldenFingerCanvasLayer overlay 隔离中延续〕
...
**影响**
- ~~画笔体验向 tldraw 官方能力靠拢。~~ 〔2026-06-02 注：tldraw 退场后该目标随框架失效，画笔/橡皮由 GoldenFingerCanvasLayer 自有 canvas 承担〕
- 保留“纪律边界”：不允许底层真相静默损坏。
