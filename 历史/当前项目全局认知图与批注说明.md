...
当前状态：
- 只是 re-export：`StagePreview as StagePreview`，实际导向 `LegacyStagePreview`

批注：
- 这是迁移的最佳切口。
- 真正的替换点不是 `App.tsx`，而是这里。
- `StagePreview.tsx -> LegacyStagePreview.tsx -> DrawboardStage.tsx / AutoHandwritingLayer.tsx` 才是当前正式主舞台路径；`TldrawStagePreview.tsx` 已降为遗留退场线。
...
flowchart TD
  ...
  I --> J[LegacyStagePreview.tsx]
  J --> K[DrawboardStage.tsx]
  J --> L[AutoHandwritingLayer.tsx]
  G --> M[TeachingTimeline / Inspector / AssetPanel]
  K --> N[coursewareChrome.ts]
  K --> O[StagePreviewToolbar.tsx]
  O --> P[StageRecorderControl.tsx]
  P --> Q[useCanvasRecorder.ts]
  L --> R[getBoardRevealProgress.ts]
  L --> S[boardStickerGeometry.ts]
  S --> T[useBoardStickerDragController.ts]
```
...
- 当前正式主舞台路径是 `StagePreview.tsx -> LegacyStagePreview.tsx -> DrawboardStage.tsx / AutoHandwritingLayer.tsx`
- `TldrawStagePreview.tsx` / `abcToTldrawShapes.ts` 已降为遗留退场线，不再拥有当前主舞台判断权
...
### 需要替换的
1. `StagePreview.tsx` 的导出目标（当前导向 `LegacyStagePreview`，未来再切 Konva 主舞台）
2. `LegacyStagePreview.tsx` 这一层主舞台适配器
3. `DrawboardStage.tsx / AutoHandwritingLayer.tsx` 与 Konva 内容层的承载边界
