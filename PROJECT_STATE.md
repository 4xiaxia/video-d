...
- 最后更新：2026-06-07 04:27:13 +08:00
- 当前阶段：分片 chrome 已收成扁平模块；固定四区 y 约束已退出 C 活链路；普通 C 改为实时文本；Konva 已确认为主舞台控制方向但主链迁移进入 gate，不盲切
- 负责人：当前 agent
...
- 当前不把 Konva proof 直接切成生产主舞台；Konva 是后续主舞台控制标准方向，但必须先通过 C realtime 内容来源、字体命中、拖拽/缩放、录制三层、金手指 overlay 和动态 bbox 等 gate。
...
- 分片容器 chrome 是扁平显示模块：标签、容器框、C 内容同用 zone key 归组，但不把 C 内容包进新的嵌套容器；容器大小继续由真实内容 bbox + 5px padding 生成。
...
- 录制 C 内容：`src/components/AutoHandwritingLayer.tsx` - 普通 C 录制改为 canvas `fillText` 实时绘制；复杂公式仍保留公式图片路线；旧 `constrainYPercentToZone` 已移出活链路。
...
- 目标：先收普通 C realtime text 的共享 layout helper 和字体命中验证；这是 Konva content-layer pilot 前置 gate。
- 目标：Konva 下一刀只能做受控 pilot：消费真实 `TimelineClip` / `problemText.summary` / `CoursewareZoneBox`，暴露可录制 content canvas，通过 gate 后再考虑改 `StagePreview` 公共入口。
- 目标：如需继续做“标签作为锚点”的完整模型，先设计独立持久化字段；不要把标签位置写回 `boardSlice`、clip label 或 C 内容字段。
...
- 关键文件：`src/components/CoursewareSegmentChrome.tsx`、`src/modules/canvasStage/coursewareZoneLayout.ts`、`src/components/AutoHandwritingLayer.tsx`、`src/components/BoardHandwritingStickerContent.tsx`、`src/modules/canvasStage/drawCoursewareStageFrame.ts`、`src/stage.css`。
