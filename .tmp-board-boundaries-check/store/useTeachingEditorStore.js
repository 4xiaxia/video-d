...
import { createSeedProject, } from '../domain/teachingProject';
import { createBoardTypographyConfig, } from '../modules/boardFont/boardFontConfig';
...
function updateAsset(assets, kind, patch) {
    const assetId = assetIdsByKind[kind];
    return assets.map((asset) => asset.id === assetId || asset.kind === kind
        ? {
            ...asset,
            ...patch,
        }
...
function updateAssetSummary(assets, kind, summary, source) {
    const patch = {
        summary,
        source,
    };
...
function createEmptyScriptAgentDraft() {
    return {
        boardPlan: '',
        spokenScript: '',
    };
}
export const useTeachingEditorStore = create((set) => {
    ...
    return {
        ...
        setLiveTimelinePlayhead: (playheadMs) => set((state) => ({
            livePlayheadMs: playheadMs === null ? null : clampNumber(playheadMs, 0, state.project.timeline.durationMs),
        })),
        setTimelinePlayhead: (playheadMs) => set((state) => {
            const project = {
                ...state.project,
                timeline: {
                    ...state.project.timeline,
        ...
        importProblemImage: (file) => set((state) => {
        ...
        updateProblemText: (text) => set((state) => {
        ...
        applyRecognizedProblemText: (text, confirm = false) => set((state) => {
            const targetProject = state.pendingProject ?? state.project;
            const project = {
                ...targetProject,
                assets: updateAsset(targetProject.assets, 'problemText', {
        ...
        updateScriptText: (text) => set((state) => {
            const targetProject = state.pendingProject ?? state.project;
            const project = {
                ...targetProject,
        ...
        updateBoardLayout: (text) => set((state) => {
            const targetProject = state.pendingProject ?? state.project;
            const project = {
                ...targetProject,
        ...
        updateScriptAgentCandidateDraft: (draft) => set((state) => {
        ...
        patchScriptAgentCandidateDraft: (patch) => set((state) => {
        ...
        applyScriptAgentDraft: (draft) => set((state) => {
            const normalizedDraft = normalizeScriptAgentDraft(draft);
            const scriptRowsSourceRef = createScriptChainKeysSourceRef(normalizedDraft.rows);
            const targetProject = state.pendingProject ?? state.project;
            const project = {
                ...targetProject,
                assets: updateAssetSummary(updateAsset(updateAssetSummary(targetProject.assets, 'scriptText', normalizedDraft.spokenScript, 'agent'), 'scriptText', {
                    sourceRef: scriptRowsSourceRef,
                }), 'boardLayout', normalizedDraft.boardPlan, 'agent'),
            };
            return {
                project: persistProject(project),
        ...
        updateVoiceAudio: (payload) => set((state) => {
        ...
        updateVoiceTiming: (payload) => set((state) => {
            const project = {
                ...state.project,
        ...
        updateStageCanvas: (canvas) => set((state) => {
            const project = {
                ...state.project,
                stage: {
                    ...state.project.stage,
        ...
        applyTtsSentenceResults: (results) => set((state) => {
            ...
            const project = {
                ...state.project,
                assets: updateAsset(updateAsset(state.project.assets, 'voiceAudio', {
                    source: 'tts',
        ...
        applyBoardEventsToTimeline: (boardEvents) => set((state) => {
            const project = {
                ...state.project,
        ...
        syncCAssetPrewarmQueue: (cAssets) => set((state) => {
            const project = {
                ...state.project,
                cAssets,
            };
        ...
        updateBoardClip: (clipId, patch) => set((state) => updateBoardClipState(state, clipId, patch)),
        updateBoardTiming: (clipId, patch) => set((state) => updateBoardClipState(state, clipId, patch)),
        updateSelectedBoardClip: (patch) => set((state) => {
...
if (typeof window !== 'undefined' && import.meta.env.DEV) {
    window.__TEACHING_EDITOR_STORE__ = useTeachingEditorStore;
}
...
function loadPersistedProject(config) {
...
function loadPersistedPendingProject(config) {
    if (typeof window === 'undefined') {
        return null;
    }
    try {
        ...
        return normalizePersistedProject(JSON.parse(rawProject), createSeedProjectFromConfig(config));
    }
    catch {
        return null;
    }
}
function normalizePersistedProject(parsedProject, seedProject) {
    const parsedTimeline = parsedProject.timeline ?? seedProject.timeline;
    const clips = Array.isArray(parsedTimeline.clips) ? parsedTimeline.clips : seedProject.timeline.clips;
    const durationMs = Math.max(seedProject.timeline.durationMs, ...clips.map((clip) => Number(clip.endMs) || 0));
    return {
...
function setProjectPersistencePlayingState(playing) {
...
function persistPendingProject(project) {
    if (typeof window !== 'undefined') {
        window.localStorage.setItem(pendingProjectStorageKey, JSON.stringify(project));
    }
    return project;
}
function clearPersistedPendingProject() {
    if (typeof window !== 'undefined') {
...
function mergeConfig(config) {
    ...
    const savedStageDefaults = savedConfig.stageDefaults ?? {};
    const savedStageCanvas = savedStageDefaults.canvas ?? {};
    const savedScriptAgent = savedConfig.scriptAgent ?? {};
    const scriptAgent = {
...
function shouldUpgradeScriptAgentPrompt(promptSystem) {
    ...
    const looksLikeOldDefaultScriptAgentPrompt = promptSystem.includes('## 断句真相') &&
...
function shouldUpgradeScriptAgentUserTemplate(promptUserTemplate) {
    return (typeof promptUserTemplate === 'string' &&
...
function hasPollutedTemplateOpenBcPrompt(text) {
...
function loadPersistedScriptAgentCandidateDraft() {
    if (typeof window === 'undefined') {
...
function persistScriptAgentCandidateDraft(draft) {
    const normalizedDraft = normalizeScriptAgentDraft(draft);
    if (typeof window === 'undefined') {
...
function clearPersistedScriptAgentCandidateDraft() {
    if (typeof window !== 'undefined') {
        window.localStorage.removeItem(scriptAgentCandidateDraftStorageKey);
    }
    return createEmptyScriptAgentDraft();
}
function shouldKeepExistingScriptAgentDraft(currentDraft, nextDraft) {
    return hasScriptAgentDraftContent(currentDraft) && !hasScriptAgentDraftContent(nextDraft);
}
function createSeedProjectFromConfig(config) {
...
function createStageCanvasFromConfig(config) {
...
function normalizeStageCanvas(canvas) {
    const boardTypography = createBoardTypographyConfig(canvas);
    return {
        ...canvas,
...
function createProjectTitleFromFileName(fileName, fallback) {
    const baseName = fileName.replace(/\.[^.]+$/, '').trim();
    return baseName || fallback;
}
function updateBoardClipState(state, clipId, patch) {
...
function normalizeBoardHideAtMs({ displayEndMs, displayStartMs, patch, previousHideAtMs, }) {
...
function createVoiceTimelineClips(results) {
    let cursorMs = 0;
