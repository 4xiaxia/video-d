...
const HANDWRITING_STRUCTURAL_MATH_PATTERN = /\\[a-zA-Z]+|[A-Za-z0-9)\]}]\s*[\^_]\s*\{[^}]+\}|[A-Za-z0-9)\]}]\s*[\^_]\s*[A-Za-z0-9]\s*[\^_]/;
const HANDWRITING_ASCII_SYMBOLS = `!"#$%&'()*+,-./:;<=>?@[\\]^_\`{|}~`;
const HANDWRITING_EXTRA_SYMBOLS = "\u201c\u201d\u2018\u2019\uff08\uff09\u3010\u3011\u300a\u300b\u3008\u3009\u300c\u300d\u300e\u300f\uff0c\u3002\uff01\uff1f\uff1b\uff1a\u3001\u2014\u2026\u00b7\uff1d\u00d7\u00f7\u2264\u2265\u2260\u2248\u2220\u00b0\u00b1\u2213\u221e\u03c0\u221d\u22a5\u2225\u25b3\u2299\u2235\u2234\u221a\u2208\u2205\u2192\u2190\u2194\u21d2\u2261\u2263\u22c5\u222b\u2200\u2203\u2282\u2283\u222a\u2229\u2032\u2033\u2103\u2109";
export function normalizeBoardMathText(text) {
    return text
        .replace(/\u000c\s*rac/g, '\\frac')
        .replace(/(^|[\s([{:=+\-×÷*/，,：:])rac(?=\s*\{)/g, '$1\\frac')
        .replace(/(^|[\s([{:=+\-×÷*/，,：:])frac(?=\s*\{)/g, '$1\\frac');
}
export function hasBoardMath(text) {
...
export function stripSimpleBoardMathDelimiters(text) {
...
export function normalizeElementaryBoardHandwritingText(text) {
    ...
    const normalizedText = strippedText
        ...
        .replace(/\\pi\b/g, 'π')
        .replace(/\\infty\b/g, '∞')
        .replace(/\\sqrt\b\s*\{([^}]*)\}/g, (_match, inner) => `√${inner}`)
        .replace(/\\sqrt\b/g, '√')
        .replace(/\\in\b/g, '∈')
        .replace(/\\notin\b/g, '∉')
        .replace(/\\subset\b/g, '⊂')
...
export function tokenizeBoardText(text) {
    const normalizedText = normalizeBoardMathText(text).trim();
    return normalizedText.split('\n').map((line) => tokenizeBoardLine(line.trim()));
}
function tokenizeBoardLine(line) {
    if (!line) {
...
function tokenizeExplicitMath(line) {
    const tokens = [];
    let cursor = 0;
...
function shouldKeepAsHandwritingText(line) {
...
// 不是判断"必须同时含有哪两类字符"。
export function isBoardTextSupportedByHandwritingFont(line) {
...
function isHandwritingSupportedChar(char) {
    return (/\s/.test(char) ||
 ...
 */
export function normalizeHandwritingDisplayText(text) {
    return text
        .replace(/×/g, 'x')
        .replace(/÷/g, '·');
}
...
function findNextExplicitMathDelimiter(line, cursor) {
    const candidates = [
...
function findImplicitMathStart(line) {
