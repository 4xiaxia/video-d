...
export function useVoiceTrackAudio({ clips, isPlaying, onSetPlayhead, onStop, playheadMs, }) {
    ...
    const playheadAnimationFrameIdRef = useRef(null);
    const [activeClipId, setActiveClipId] = useState(null);
    const [error, setError] = useState('');
    const playableClips = useMemo(() => clips
        .filter((clip) => clip.kind === 'audio' && Boolean(clip.sourceRef))
        .sort((left, right) => left.startMs - right.startMs), [clips]);
    ...
    const startPlayheadTimer = useCallback((clip, offsetMs) => {
        stopPlayheadTimer();
    ...
    useEffect(() => {
        ...
        const playClip = async (clip, offsetMs) => {
            setError('');
        ...
        const handleEnded = () => {
            ...
            const nextClip = currentIndex >= 0 ? playableClips[currentIndex + 1] : findNextPlayableClip(playheadRef.current + 1, playableClips);
            if (!nextClip) {
                const finalPlayheadMs = currentClip?.endMs ?? playableClips[playableClips.length - 1]?.endMs ?? playheadRef.current;
...
function findNextPlayableClip(playheadMs, clips) {
    return clips.find((clip) => clip.endMs > playheadMs);
}
function resolveAudioSource(sourceRef) {
    if (!sourceRef) {
        return '';
    }
