...
export function createTtsBatchJobs(units, options = {}) {
    ...
    return splitBalancedByCount(units, minSentencesPerBatch, maxSentencesPerBatch, maxBatchDurationMs).map((batch, index) => toBatchJob(batch, index + 1, maxBatchDurationMs, concurrencyLimit));
}
function splitBalancedByCount(units, minSentencesPerBatch, maxSentencesPerBatch, maxBatchDurationMs) {
    if (units.length === 0)
        return [];
...
function splitByDurationFallback(units, maxSentencesPerBatch, maxBatchDurationMs) {
    const batches = [];
    let currentBatch = [];
...
function toBatchJob(units, batchIndex, maxDurationMs, concurrencyLimit) {
    return {
...
function sumEstimatedDurationMs(units) {
