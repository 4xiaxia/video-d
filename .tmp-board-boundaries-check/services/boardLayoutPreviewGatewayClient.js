export async function requestBoardLayoutPreview({ config, problemText, rows, stageCanvas, }) {
    if (!rows.length) {
    ...
    try {
        ...
        const payload = await readGatewayJson(response, '板书排版预览');
...
function normalizePreviewDraft(preview) {
    return {
