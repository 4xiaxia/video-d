...
import { DEFAULT_BOARD_STICKER_WIDTH_PERCENT, DEFAULT_BOARD_STICKER_X_PERCENT, DEFAULT_BOARD_STICKER_Y_PERCENT, getBoardStickerFontSize, renderBoardMathStickerImage, renderBoardTextStickerImage, resolveBoardTextDisplayRoute, useBoardStickerDragController, } from '../modules/boardSticker';
 ...
 */
function getZoneNameFromChainKey(chainKey) {
 ...
 */
function constrainYPercentToZone(yPercent, widthPercent, zone) {
...
export function AutoHandwritingLayer({ boardClips, boardFontLoadKey, boardFontSize, canvas, playheadMs, selectedBoardClipId, onRecordingCanvasReady, onSelectBoardClip, onUpdateBoardClip, }) {
    const boardAreaRef = useRef(null);
    const frozenRevealRef = useRef(null);
    ...
    useEffect(() => {
        ...
        void drawRecordingBoardContent({
            ...
            clips: visibleBoardClips.map((clip) => {
                ...
                const revealProgress = draggingClipId === clip.id && frozenRevealRef.current?.clipId === clip.id
    ...
    return (_jsx("div", { className: "courseware-board-area", ref: boardAreaRef, children: visibleBoardClips.length
            ? visibleBoardClips.map((clip, index) => {
                ...
                const revealProgress = draggingClipId === clip.id && frozenRevealRef.current?.clipId === clip.id
                    ? frozenRevealRef.current.progress
                    : liveRevealProgress;
                return (_jsx(BoardTextSticker, { color: color, isDragging: draggingClipId === clip.id, isSelected: selectedBoardClipId === clip.id, onPointerDown: (event) => {
                    ...
                    }, stackIndex: index, fontFamily: canvas.boardFontFamily, fontLoadKey: boardFontLoadKey, fontSize: fontSize, revealProgress: revealProgress, text: clip.label.trim(), widthPercent: widthPercent, xPercent: xPercent, yPercent: yPercent }, clip.id));
            })
            : null }));
}
async function drawRecordingBoardContent({ boardArea, boardFontSize, canvas, clips, recordingCanvas, shouldContinue, }) {
    if (!recordingCanvas) {
        return;
    }
...
function drawImageWithRevealClip(context, image, rect) {
...
function loadRecordingImage(src) {
...
function readBoardClipRevealProgress(clip, playheadMs) {
